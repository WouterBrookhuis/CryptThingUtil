...
bool CTU_DecryptFile(std::string const &InFile, std::string const &OutFile, std::string const &Password)
{
	...
	if (CTU_GetHeaderVersion(header) != 0 ||
...
bool CTU_EncryptFile(std::string const &InFile, std::string const &OutFile, std::string const &Password)
{
	...
	if (!CTU_EncryptData(in, out, keys, &keys[16], &encryptedFileLength, &inputFileLength)) {
		out.clear();
		in.close();
		out.close();
		return false;
...
int main()
{
	...
	std::cout << "File Crypto Util - Version: v0 - Mode: AES128GCM\r\n";
